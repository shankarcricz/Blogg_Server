...
exports.login = asyncCatch(async (req, res, next) => {
  ...
  if (!user || !(await user.passwordCheck(password, user.password))) { //the instance methods are only accessible through query & not by documents
...
exports.protect = asyncCatch(async (req,res,next) => {
  ...
  if(!currUser) return next(new AppError('user is not there for this jwt!', 401))
  
  //we check if the user has changed password after login since on password change the existing jwt makes no sense
  if(currUser.PasswordChangedAfter(decoded.iat)) return next(new AppError('password has been changed!. Pls login again', 401));
...
exports.forgotPassword = asyncCatch(async (req, res , next) => {
  ...
  exports.resetPassword = asyncCatch(async (req, res, next) => {
    ...
    const hashedToken = crypto.createHash('sha256').update(userToken).digest('hex') //hashing the token from user & checking if it is there in db
    const user = await User.findOne({passwordResetToken : hashedToken, passwordResetExpires : {$gt :  Date.now()}}); //also checking if the password is expired or not
  ...
  exports.updatePassword = asyncCatch(async(req, res, next) => {
    ...
    if(!user.passwordCheck(req.body.passwordCurrent, user.password)) return next(new AppError("Wrong password!"))
    user.password = req.body.password;
    user.passwordConfirm = req.body.passwordConfirm;
    await user.save();
